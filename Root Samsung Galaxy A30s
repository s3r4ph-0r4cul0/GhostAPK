# Root Samsung Galaxy A30s (Magisk + Odin4 – Linux)
...
* **Modelo**: Samsung Galaxy A30s (SM-A307*)
* **Plataforma**: Exynos
...
4. **Desmarque a opção "Recovery Mode"** (em alguns casos)

> ℹ️ Observação: na maioria dos dispositivos Samsung o patch correto é **SEM Recovery Mode**. Caso o root **não funcione ao final do processo**, repita o patch **com o Recovery Mode ativado** como teste alternativo.
...
> Neste cenário, **não é obrigatório** carregar BL, CP e CSC.

### Cenário B — Patch **COM Recovery Mode**
...
4. Em **Options**:
   ...
   * ❌ Auto Reboot (desmarcado)
   * ❌ Re-Partition (desmarcado)

5. Clique em **Start** e aguarde **PASS**
...
### Caso o patch tenha sido feito **SEM Recovery Mode**

* O aparelho pode **reiniciar normalmente**

### Caso o patch tenha sido feito **COM Recovery Mode**

1. **Forçar reboot manual**:

   * Segure **Power + Volume -** até desligar
2. **Imediatamente** segure:

   * **Power + Volume +**
3. Solte ao aparecer o logo Samsung

Isso garante que o **Magisk seja inicializado corretamente**.
