...
Java.perform(function () {
  ...
  try {
    Java.use("java.io.File").exists.implementation = function () {
      ...
      var catched =
        ["qemud", "qemu_pipe", "drivers", "cpuinfo"].indexOf(name) > -1;
      if (catched) {
  ...
  try {
    Java.use("android.app.ApplicationPackageManager").getPackageInfo.overload(
      ...
      var catched =
        ["com.example.android.apis", "com.android.development"].indexOf(name) >
        -1;
      if (catched) {
...
setTimeout(function () {
  ...
  function bypassNativeFileCheck() {
    try {
      ...
      if (fopen) {
          ...
          onLeave: function (retval) {
            try {
              if (retval && retval.toInt32() != 0) {
                if (this.inputPath && commonPaths.indexOf(this.inputPath) >= 0) {
        ...
        console.log("[-] fopen export not found");
      }
      ...
      console.log("[-] Error in bypassNativeFileCheck: " + error);
    }
  }
  ...
  Java.perform(function () {
    ...
    if (fopen) {
      ...
      console.log("[-] fopen not found for root bypass");
    }
